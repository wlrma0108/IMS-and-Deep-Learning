...
class SimpleResize:
    """검증용: 480×480 Resize"""
    def __init__(self, out_size=480):
...
train_transform = StrongAugment(output_size=480)
val_transform   = SimpleResize(out_size=480)
...
def fit(epochs, model, train_loader, val_loader, criterion, optimizer, scheduler):
    ...
    for e in range(epochs):
        ...
        if val_loss < min_val_loss:
            ...
            no_improve   = 0
            torch.save(model.state_dict(), f"Unet_effb5_epoch{e+1}_val{val_loss:.3f}.pth")
        else:
            no_improve+=1
