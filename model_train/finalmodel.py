...
class RandomRotationCropFlip:
    ...
    def __call__(self, img, mask):
        ...
        if (w > crop_w) and (h > crop_h):
            ...
            img  = TF.crop(img, top, left, min(h, crop_h), min(w, crop_w))
            mask = TF.crop(mask, top, left, min(h, crop_h), min(w, crop_w))
...
class MySegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        print("image_3ch.shape:", image_3ch.shape)
        print("dtype:", image_3ch.dtype)
        print("min, max:", image_3ch.min(), image_3ch.max())
        pil_img = Image.fromarray(image_3ch, mode='RGB')
    ...
    # (H,W)->(H,W,3)
        if image.ndim == 2:
            image_3ch = np.stack([image, image, image], axis=-1)  # 단일 채널 복제
        else:
            image_3ch = image  # 이미 3채널이면 그대로

    # PIL 변환 (mode='RGB'로)
    # float이면 0..1 사이인지 체크하거나, 0..255 범위면 uint8로 변환
        image_3ch = (image_3ch*255).clip(0,255).astype(np.uint8)  # 예시
        pil_img   = Image.fromarray(image_3ch, mode='RGB')
        ...
        t_img = T.ToTensor()(pil_img)  # shape (3, H, W)
    # 사전학습 Imagenet에 맞게 Normalize
        t_img = T.Normalize([0.485, 0.456, 0.406],
                        [0.229, 0.224, 0.225])(t_img)
    
        t_mask= torch.from_numpy(np.array(pil_mask, dtype=np.int64)) # (H, W)
...
def visualize_samples(dataloader, n=2):
    ...
    imgs, msks = next(data_iter)  # (B,1,H,W), (B,H,W)
...
def fit(epochs, model, train_loader, val_loader, criterion, optimizer, scheduler):
    ...
    for e in range(epochs):
        model.train()
        train_loss, train_acc, train_iou = 0.0,0.0,0.0
        ...
        # Valid
        model.eval()
        val_loss, val_acc, val_iou = 0.0,0.0,0.0
        with torch.no_grad():
