...
# 1) .npy 파일 로드 & 전처리
# --------------------------------------------------------------------------------
prefix = "C:\\Users\\admin\\dataset"  # 실제 경로에 맞춰 수정
...
train_transform = StrongAugment(output_size=256)
...
def visualize_samples(dataloader, n=2):
    ...
    for i in range(min(n, len(imgs))):
        # (3,H,W)->(H,W,3)
        img_np  = imgs[i].cpu().numpy().transpose(1,2,0)  # shape (H,W,3)
        # Normalize 해제(시각적 확인 목적) => optional
        # for c in range(3):
        #     img_np[..., c] = img_np[..., c]*self.std[c] + self.mean[c]
        # (mask)
        mask_np = msks[i].cpu().numpy()

        fig, ax = plt.subplots(1,2, figsize=(8,4))
        ax[0].imshow(img_np.astype(np.float32), cmap=None)  # 실제 RGB
...
model = smp.Unet(
    ...
    classes=4,  
    activation=None,
    decoder_dropout=0.2 # 정규화를 위한 수정(김대엽)
)
